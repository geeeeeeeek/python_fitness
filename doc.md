...
一直想做一款健身房管理系统，看了很多优秀的开源项目但是发现没有合适的。于是利用空闲休息时间开始自己写了一套管理系统。
学习过程中遇到问题可以咨询微信：lengqin1024 或者发Email: kefu308@gmail.com

## 在线体验


- 代码下载：https://github.com/geeeeeeeek/python_fitness
- 演示地址：http://fitness.gitapp.cn/
...
- 教练管理：管理系统可以录入、修改和查询教练的基本信息，如教练名称、价格、备注等。
- 场地管理：场地信息的增删改查。
- 会员管理：会员信息的增删改查。
- 课程管理：课程信息的增删改查。
- 器材管理：器材信息的增删改查。
...
在教练管理系统中，需要存储和管理教练信息、评论信息、分类信息、标签信息、用户信息、通知信息、日志信息。
...
无论是教练管理、用户管理、场地管理、课程管理、日志管理、消息管理等功能都是基于springboot+vue框架开发的，开发流程是：
...
下面用教练管理功能来演绎这个流程，其它的管理功能都是这个流程。
      ...
      >
        ...
        <div style="padding-right: 16px; max-height: 480px; overflow-x: hidden; overflow-y: auto">
          <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
            <a-row :gutter="24">
              <a-col span="12">
                <a-form-item label="场地" name="classification">
                  <a-select
                    placeholder="请选择"
                    allowClear
                    :options="modal.cData"
              ...
              <a-col span="12">
                <a-form-item label="专长" name="major">
                  <a-input placeholder="请输入" v-model:value="modal.form.major" />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
  ...
  const handleOk = () => {
    myform.value
      ...
      .then(() => {
        ...
        formData.append('major', modal.form.major || '');
...
```
这就是教练管理功能的实现流程，其它的功能管理实现一模一样的。按照这个流程编写即可。
